...
__global__ void _kernel_redundant_bounds_candidates(std::uint8_t* drop_lower,
  ...
  if (j < n) {
    ...
    const bool doubly_bounded = isfinite(lj) && isfinite(uj);
    // Restrict doubly bounded candidates to zero-cost auxiliaries to avoid
    // changing objective geometry when removing implied box projections.
    const bool allow_doubly_bounded =
        doubly_bounded && (c == nullptr || fabs(c[j]) <= zero_tol);
    const bool eligible = allow_doubly_bounded ||
                          (allow_one_sided != 0 && !doubly_bounded);

    // Check each finite side independently. Batched removal certificates must
    // not depend on other selected columns in the same row.
    if (isfinite(uj) && eligible) {
...
__global__ void _kernel_apply_selected_redundant_bounds(double* l_cur,
  ...
  if (j < n) {
    ...
    if (selected_upper[j] != std::uint8_t{0}) {
      u_cur[j] = INFINITY;
    }
  }
}
...
void apply_rule_redundant_bounds(PresolvePlanGpu& plan,
  ...
  for (;;) {
    ...
    _kernel_redundant_bounds_candidates<<<blocks_n, GPU_PRESOLVE_THREADS>>>(
        ...
        lp.AT.colVal,
        lp.AT.nzVal,
        pparams.enable_redundant_bounds ? 1 : 0,
        pparams.zero_tol,
        pparams.bound_tol,
        n);
    ...
    if (pparams.record_postsolve_tape) {
      ...
      for (std::int32_t col = 0; col < n; ++col) {
        ...
        if (host_selected_lower[idx] != std::uint8_t{0} ||
            host_selected_upper[idx] != std::uint8_t{0}) {
          const double new_l = host_selected_lower[idx] != std::uint8_t{0}
                                   ? -INFINITY
                                   : host_old_l[idx];
          const double new_u = host_selected_upper[idx] != std::uint8_t{0}
                                   ? INFINITY
                                   : host_old_u[idx];
          append_postsolve_record(plan.tape,
                                  PostsolveReductionType::BoundChangeNoRow,
                                  {col},
                                  {host_old_l[idx], host_old_u[idx], new_l, new_u},
