...
__global__ void _kernel_infeasible_redundant_rows_classify(std::int32_t* status_flags,
...
__global__ void _kernel_infeasible_redundant_rows_apply(std::uint8_t* keep_row,
                                              double* AL,
                                              double* AU,
...
void apply_rule_infeasible_redundant_rows(PresolvePlanGpu& plan,
  ...
  throw_if_cuda_error(cudaMalloc(&status_flags, sizeof(std::int32_t) * 4), "cudaMalloc infeasible_redundant_rows status_flags");
  throw_if_cuda_error(cudaMalloc(&full_redundant, static_cast<std::size_t>(m)), "cudaMalloc infeasible_redundant_rows full_redundant");
  throw_if_cuda_error(cudaMalloc(&drop_lower, static_cast<std::size_t>(m)), "cudaMalloc infeasible_redundant_rows drop_lower");
  throw_if_cuda_error(cudaMalloc(&drop_upper, static_cast<std::size_t>(m)), "cudaMalloc infeasible_redundant_rows drop_upper");
  throw_if_cuda_error(cudaMemset(status_flags, 0, sizeof(std::int32_t) * 4), "cudaMemset infeasible_redundant_rows status_flags");

  const int blocks = (m + GPU_PRESOLVE_THREADS - 1) / GPU_PRESOLVE_THREADS;
  _kernel_infeasible_redundant_rows_classify<<<blocks, GPU_PRESOLVE_THREADS>>>(
      status_flags,
      full_redundant,
      drop_lower,
      drop_upper,
      ...
      pparams.bound_tol,
      m);
  throw_if_cuda_error(cudaGetLastError(), "_kernel_infeasible_redundant_rows_classify");

  std::int32_t flags[4] = {0, 0, 0, 0};
  throw_if_cuda_error(cudaMemcpy(flags, status_flags, sizeof(flags), cudaMemcpyDeviceToHost),
                      "cudaMemcpy infeasible_redundant_rows flags");
  if (flags[0] != 0) {
    ...
    _kernel_infeasible_redundant_rows_apply<<<blocks, GPU_PRESOLVE_THREADS>>>(
        plan.keep_row_mask,
        plan.new_AL,
        plan.new_AU,
        full_redundant,
        ...
        drop_upper,
        m);
    throw_if_cuda_error(cudaGetLastError(), "_kernel_infeasible_redundant_rows_apply");
    throw_if_cuda_error(cudaDeviceSynchronize(), "apply_rule_infeasible_redundant_rows synchronize");
