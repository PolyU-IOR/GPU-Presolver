...
__global__ void _kernel_infeasible_fixed_variables_candidates(std::uint8_t* fixed_mask,
                                                double* fixed_val,
                                                double* obj_contrib,
                                                double* row_shift,
...
__global__ void _kernel_infeasible_fixed_variables_apply(std::uint8_t* keep_col,
                                           double* new_AL,
                                           double* new_AU,
                                           double* new_l,
                                           double* new_u,
  ...
  if (j < n && fixed_mask[j] != std::uint8_t{0}) {
    // The generic postsolve recorder recognizes fixed columns from equal
    // bounds. Preserve the exact value used for the row/objective shifts.
    new_l[j] = fixed_val[j];
    new_u[j] = fixed_val[j];
...
void apply_rule_infeasible_fixed_variables(PresolvePlanGpu& plan,
  ...
  _kernel_infeasible_fixed_variables_candidates<<<blocks_n, GPU_PRESOLVE_THREADS>>>(
      fixed_mask,
      fixed_val,
      obj_contrib,
      row_shift,
      ...
      pparams.bound_tol,
      n);
  throw_if_cuda_error(cudaGetLastError(), "_kernel_infeasible_fixed_variables_candidates");

  _kernel_count_mask<<<blocks_n, GPU_PRESOLVE_THREADS>>>(
      fixed_mask,
      n,
      fixed_count_device);
  throw_if_cuda_error(cudaGetLastError(), "_kernel_count_mask infeasible_fixed_variables");
  ...
  _kernel_infeasible_fixed_variables_apply<<<blocks_mn, GPU_PRESOLVE_THREADS>>>(
      plan.keep_col_mask,
      plan.new_AL,
      plan.new_AU,
      plan.new_l,
      ...
      n,
      m);
  throw_if_cuda_error(cudaGetLastError(), "_kernel_infeasible_fixed_variables_apply");
  throw_if_cuda_error(cudaDeviceSynchronize(), "apply_rule_infeasible_fixed_variables synchronize");

  const double obj_delta = sum_device_double(obj_contrib, n, "cudaMemcpy infeasible_fixed_variables obj_contrib");
