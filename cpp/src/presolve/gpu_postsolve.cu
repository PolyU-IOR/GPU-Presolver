...
__global__ void _kernel_cache_antipodal_root_values(
    double* plus_cache,
    double* minus_cache,
    const double* x_org,
    const std::int32_t* root_plus_cols,
    const std::int32_t* root_minus_cols,
    std::int32_t count) {
  const std::int32_t i =
      static_cast<std::int32_t>(blockIdx.x * blockDim.x + threadIdx.x);
  if (i < count) {
    plus_cache[i] = x_org[root_plus_cols[i]];
    minus_cache[i] = x_org[root_minus_cols[i]];
  }
}

__global__ void _kernel_restore_antipodal_component_members(
    double* x_org,
    const std::int32_t* plus_cols,
    const std::int32_t* minus_cols,
    const double* plus_lower,
    const double* minus_lower,
    const double* plus_cache,
    const double* minus_cache,
    std::int32_t count) {
  const std::int32_t i =
      static_cast<std::int32_t>(blockIdx.x * blockDim.x + threadIdx.x);
  if (i < count) {
    x_org[plus_cols[i]] = plus_lower[i] + plus_cache[i];
    x_org[minus_cols[i]] = minus_lower[i] + minus_cache[i];
  }
}
...
__device__ double _duplicate_column_aggregate_lower_bound(double target_l,
                                                      double source_l,
                                                      double source_u,
                                                      double ratio) {
  if (ratio > 0.0) {
...
__device__ double _duplicate_column_aggregate_upper_bound(double target_u,
                                                      double source_l,
                                                      double source_u,
                                                      double ratio) {
  if (ratio > 0.0) {
...
__device__ void _recover_duplicate_column_primal(double y_val,
                                             double z_to,
                                             double ratio,
                                             double source_l,
                                             double source_u,
  ...
  const double lower = _duplicate_column_aggregate_lower_bound(target_l, source_l, source_u, ratio);
  const double upper = _duplicate_column_aggregate_upper_bound(target_u, source_l, source_u, ratio);
...
__device__ void _duplicate_row_scaled_interval(double lower,
                                              double upper,
                                              double ratio,
                                              double* scaled_l,
                                              double* scaled_u) {
...
__device__ bool _duplicate_row_interval_contains(double outer_lo,
                                                double outer_hi,
                                                double inner_lo,
                                                double inner_hi,
                                                double tol) {
...
__device__ double _duplicate_row_activity(std::int32_t row,
...
__device__ std::uint8_t _duplicate_row_dual_state(double activity,
                                                 double lower,
                                                 double upper,
                                                 double tol) {
...
__device__ void _duplicate_row_state_interval(std::uint8_t state, double* lo, double* hi) {
...
__device__ void _duplicate_row_deleted_dual_interval(double ybar_p,
                                                    double ratio,
...
__device__ bool _duplicate_row_doc_dual_split(double ybar_p,
                                             double ratio,
                                             double kept_old_AL,
                                             double kept_old_AU,
...
__device__ void _doubleton_equations_recover_z_split(double x_keep,
                                              double z_keep_hat,
                                              double alpha,
                                              double old_keep_l,
                                              double old_keep_u,
...
__device__ double _doubleton_equations_deleted_row_dual_from_original(const double* y_org,
                                                               const std::int32_t* AT_rowPtr,
                                                               const std::int32_t* AT_colVal,
                                                               const double* AT_nzVal,
                                                               const double* c,
...
__device__ double _doubleton_equations_deleted_row_dual_from_tape(const double* y_org,
...
__global__ void _kernel_replay_postsolve_tape(double* x_org,
                                              ...
                                              const double* l,
                                              const double* u,
                                              std::int32_t record_begin,
                                              std::int32_t record_end,
                                              std::int32_t has_original_model,
                                              std::int32_t replay_primal,
                                              std::int32_t replay_dual,
                                              std::int32_t duplicate_row_doc_recovery,
                                              double tol) {
  if (blockIdx.x != 0 || threadIdx.x != 0) {
    return;
  }
  for (std::int32_t k = record_end - 1; k >= record_begin; --k) {
    ...
    if (reduction_type == static_cast<std::int32_t>(PostsolveReductionType::DuplicateColumn)) {
      ...
      if (replay_primal != 0) {
        ...
        double x_from = 0.0;
        double x_to = y_val;
        _recover_duplicate_column_primal(y_val, z_to, ratio, lo, hi, to_l, to_u, tol, &x_from, &x_to);
    ...
    } else if (reduction_type == static_cast<std::int32_t>(PostsolveReductionType::DoubletonEquation)) {
      ...
      if (replay_dual != 0 && z_org != nullptr && y_org != nullptr && has_original_model != 0) {
        ...
        double z_keep = 0.0;
        double z_elim = 0.0;
        _doubleton_equations_recover_z_split(x_org[keep_col],
                                      z_org[keep_col],
                                      alpha,
                                      old_keep_l,
                                      old_keep_u,
        ...
        y_org[row] = has_support_payload
                         ? _doubleton_equations_deleted_row_dual_from_tape(
                               y_org, tape_idx, tape_val, idx0, val0, elim_coeff, z_elim)
                         : _doubleton_equations_deleted_row_dual_from_original(
      ...
      if (y_org != nullptr) {
        ...
        y_org[row] += tape_val[val0];
               ...
               reduction_type == static_cast<std::int32_t>(PostsolveReductionType::DuplicateRow) &&
      ...
      _duplicate_row_scaled_interval(deleted_old_AL, deleted_old_AU, ratio,
                                    &deleted_scaled_AL, &deleted_scaled_AU);
      if (_duplicate_row_interval_contains(deleted_scaled_AL,
                                          deleted_scaled_AU,
                                          kept_old_AL,
                                          kept_old_AU,
                                          tol)) {
      ...
      if (has_original_model != 0) {
        const double alpha_p = _duplicate_row_activity(kept_row, A_rowPtr, A_colVal, A_nzVal, x_org);
        const double alpha_d = _duplicate_row_activity(deleted_row, A_rowPtr, A_colVal, A_nzVal, x_org);
        const std::uint8_t kept_state =
            _duplicate_row_dual_state(alpha_p, kept_old_AL, kept_old_AU, tol);
        const std::uint8_t deleted_state =
            _duplicate_row_dual_state(alpha_d, deleted_old_AL, deleted_old_AU, tol);
        double yd_lo = 0.0;
        double yd_hi = 0.0;
        double yp_lo = 0.0;
        double yp_hi = 0.0;
        _duplicate_row_state_interval(deleted_state, &yd_lo, &yd_hi);
        _duplicate_row_deleted_dual_interval(y_org[kept_row], ratio, kept_state, &yp_lo, &yp_hi);
        ...
        if (duplicate_row_doc_recovery != 0) {
          double yp_doc = y_org[kept_row];
          double yd_doc = 0.0;
          if (_duplicate_row_doc_dual_split(y_org[kept_row],
                                           ratio,
                                           kept_old_AL,
                                           kept_old_AU,
                                           deleted_scaled_AL,
...
void restore_structural_primal_step(
    double* x_org,
    const StructuralL1PrimalRecoveryStep& recovery,
    const double* original_l) {
  restore_splits(x_org, recovery.splits);
  restore_outer_pairs(x_org, recovery.outer_pairs);
  restore_linked_slacks(x_org, recovery.linked_slacks);
  restore_max_slacks(x_org, recovery.max_slacks, original_l);
}

void restore_antipodal_component(
    double* x_org,
    const AntipodalComponentPrimalRecoveryStep& recovery) {
  const std::size_t size = recovery.plus_cols.size();
  if (size == 0) {
    return;
  }
  const std::int32_t count = static_cast<std::int32_t>(size);
  std::int32_t* plus_cols =
      copy_vector_to_device(recovery.plus_cols,
                            "cudaMalloc/copy antipodal plus cols");
  std::int32_t* minus_cols =
      copy_vector_to_device(recovery.minus_cols,
                            "cudaMalloc/copy antipodal minus cols");
  std::int32_t* root_plus_cols =
      copy_vector_to_device(recovery.root_plus_cols,
                            "cudaMalloc/copy antipodal root plus cols");
  std::int32_t* root_minus_cols =
      copy_vector_to_device(recovery.root_minus_cols,
                            "cudaMalloc/copy antipodal root minus cols");
  double* plus_lower =
      copy_vector_to_device(recovery.plus_lower,
                            "cudaMalloc/copy antipodal plus lower");
  double* minus_lower =
      copy_vector_to_device(recovery.minus_lower,
                            "cudaMalloc/copy antipodal minus lower");
  double* plus_cache = nullptr;
  double* minus_cache = nullptr;
  throw_if_cuda_error(cudaMalloc(&plus_cache, sizeof(double) * size),
                      "cudaMalloc antipodal plus cache");
  throw_if_cuda_error(cudaMalloc(&minus_cache, sizeof(double) * size),
                      "cudaMalloc antipodal minus cache");
  const int blocks =
      (count + GPU_PRESOLVE_THREADS - 1) / GPU_PRESOLVE_THREADS;
  _kernel_cache_antipodal_root_values<<<blocks, GPU_PRESOLVE_THREADS>>>(
      plus_cache, minus_cache, x_org, root_plus_cols, root_minus_cols, count);
  throw_if_cuda_error(cudaGetLastError(),
                      "_kernel_cache_antipodal_root_values");
  _kernel_restore_antipodal_component_members<<<blocks,
                                                GPU_PRESOLVE_THREADS>>>(
      x_org, plus_cols, minus_cols, plus_lower, minus_lower, plus_cache,
      minus_cache, count);
  throw_if_cuda_error(cudaGetLastError(),
                      "_kernel_restore_antipodal_component_members");
  cudaFree(plus_cols);
  cudaFree(minus_cols);
  cudaFree(root_plus_cols);
  cudaFree(root_minus_cols);
  cudaFree(plus_lower);
  cudaFree(minus_lower);
  cudaFree(plus_cache);
  cudaFree(minus_cache);
}
...
struct DevicePostsolveTape {
  ...
  ~DevicePostsolveTape() {
    if (owns_storage) {
      cudaFree(types);
      cudaFree(index_starts);
      cudaFree(value_starts);
      cudaFree(indices);
      cudaFree(vals);
    }
  }
...
void upload_postsolve_tape(const PostsolveTape& tape, DevicePostsolveTape& device_tape) {
  device_tape.count = static_cast<std::int32_t>(tape.types.size());
  if (device_tape.count == 0) {
    return;
  }
  device_tape.owns_storage = true;
  device_tape.types = copy_vector_to_device(tape.types, "cudaMalloc/copy postsolve tape types");
  device_tape.index_starts =
      copy_vector_to_device(tape.index_starts, "cudaMalloc/copy postsolve tape index_starts");
  device_tape.value_starts =
      copy_vector_to_device(tape.value_starts, "cudaMalloc/copy postsolve tape value_starts");
  device_tape.indices = copy_vector_to_device(tape.indices, "cudaMalloc/copy postsolve tape indices");
  device_tape.vals = copy_vector_to_device(tape.vals, "cudaMalloc/copy postsolve tape vals");
}

void borrow_postsolve_tape(const PostsolveTapeGpu& tape,
                           DevicePostsolveTape& device_tape) {
  device_tape.types = tape.types;
  device_tape.index_starts = tape.index_starts;
  device_tape.value_starts = tape.value_starts;
  device_tape.indices = tape.indices;
  device_tape.vals = tape.vals;
  device_tape.count = tape.record_count;
  device_tape.owns_storage = false;
}

void replay_postsolve_tape_range(double* x_org,
                                 double* y_org,
                                 double* z_org,
                                 std::uint8_t* z_retrieved,
                                 const DevicePostsolveTape& tape,
  ...
  if (record_begin < 0 || record_end < record_begin ||
      record_end > tape.count) {
    throw std::runtime_error("invalid postsolve tape replay range");
  }
...
void replay_postsolve_tape(double* x_org,
                           ...
                           bool replay_dual,
                           double tol) {
  replay_postsolve_tape_range(x_org, y_org, z_org, z_retrieved, tape,
                              original_model, 0, tape.count, replay_primal,
                              replay_dual, tol);
}
...
std::vector<std::uint8_t> build_exact_replay_column_mask(const PostsolveTape& tape, std::int32_t n) {
  ...
  for (std::size_t k = 0; k < tape.types.size(); ++k) {
    ...
    } else if (type == PostsolveReductionType::DoubletonEquation) {
    ...
    } else if (type == PostsolveReductionType::DuplicateColumn) {
      const std::int32_t col = tape.indices[idx0];
      if (0 <= col && col < n) {
        protected_cols[static_cast<std::size_t>(col)] = std::uint8_t{1};
      }
...
void validate_postsolve_tape(const PostsolveTape& tape) {
  const std::size_t count = tape.types.size();
  if (count > static_cast<std::size_t>(
                  std::numeric_limits<std::int32_t>::max())) {
    throw std::runtime_error("postsolve tape exceeds 32-bit replay range");
  }
  if (tape.index_starts.size() != count + 1 ||
      tape.value_starts.size() != count + 1 ||
      tape.dual_modes.size() != count) {
    throw std::runtime_error("malformed postsolve tape offsets");
  }
  if (tape.index_starts.front() != 0 || tape.value_starts.front() != 0) {
    throw std::runtime_error("postsolve tape offsets must start at zero");
  }
  for (std::size_t k = 0; k < count; ++k) {
    if (tape.index_starts[k] < 0 ||
        tape.index_starts[k] > tape.index_starts[k + 1] ||
        tape.value_starts[k] < 0 ||
        tape.value_starts[k] > tape.value_starts[k + 1]) {
      throw std::runtime_error("postsolve tape offsets are not monotone");
    }
    if (tape.types[k] <
            static_cast<std::int32_t>(PostsolveReductionType::FixedCol) ||
        tape.types[k] >
            static_cast<std::int32_t>(PostsolveReductionType::FmeCol)) {
      throw std::runtime_error("postsolve tape contains an unknown reduction");
    }
    if (tape.dual_modes[k] >
        static_cast<std::uint8_t>(PostsolveDualMode::Minimal)) {
      throw std::runtime_error("postsolve tape contains an unknown dual mode");
    }
  }
  if (static_cast<std::size_t>(tape.index_starts.back()) !=
          tape.indices.size() ||
      static_cast<std::size_t>(tape.value_starts.back()) != tape.vals.size()) {
    throw std::runtime_error("postsolve tape terminal offsets do not match data");
  }
}

void validate_recovery_col(std::int32_t col,
                           std::int32_t n0,
                           const char* context) {
  if (col < 0 || col >= n0) {
    throw std::runtime_error(std::string(context) +
                             " contains an invalid original column");
  }
}

void validate_structural_recovery(
    const StructuralL1PrimalRecoveryStep& recovery,
    std::int32_t n0) {
  for (const StructuralL1SplitRecovery& split : recovery.splits) {
    validate_recovery_col(split.t_col, n0, "structural split recovery");
    validate_recovery_col(split.e_col, n0, "structural split recovery");
  }
  for (const StructuralOuterPairRecovery& pair : recovery.outer_pairs) {
    validate_recovery_col(pair.bound_col, n0,
                          "structural outer-pair recovery");
    validate_recovery_col(pair.free_col, n0,
                          "structural outer-pair recovery");
  }
  for (const StructuralLinkedSlackRecovery& slack :
       recovery.linked_slacks) {
    validate_recovery_col(slack.slack_col, n0,
                          "structural linked-slack recovery");
    validate_recovery_col(slack.t_col, n0,
                          "structural linked-slack recovery");
  }
  for (const StructuralMaxSlackRecovery& slack : recovery.max_slacks) {
    if (slack.t_cols.size() != slack.factors.size()) {
      throw std::runtime_error(
          "structural max-slack recovery has mismatched payloads");
    }
    validate_recovery_col(slack.slack_col, n0,
                          "structural max-slack recovery");
    for (const std::int32_t col : slack.t_cols) {
      validate_recovery_col(col, n0, "structural max-slack recovery");
    }
  }
}

void validate_antipodal_recovery(
    const AntipodalComponentPrimalRecoveryStep& recovery,
    std::int32_t n0) {
  const std::size_t count = recovery.plus_cols.size();
  if (recovery.minus_cols.size() != count ||
      recovery.root_plus_cols.size() != count ||
      recovery.root_minus_cols.size() != count ||
      recovery.plus_lower.size() != count ||
      recovery.minus_lower.size() != count ||
      count > static_cast<std::size_t>(
                  std::numeric_limits<std::int32_t>::max())) {
    throw std::runtime_error("antipodal recovery has mismatched payloads");
  }
  for (std::size_t i = 0; i < count; ++i) {
    validate_recovery_col(recovery.plus_cols[i], n0,
                          "antipodal recovery");
    validate_recovery_col(recovery.minus_cols[i], n0,
                          "antipodal recovery");
    validate_recovery_col(recovery.root_plus_cols[i], n0,
                          "antipodal recovery");
    validate_recovery_col(recovery.root_minus_cols[i], n0,
                          "antipodal recovery");
  }
}

void validate_primal_recovery_timeline(const PresolveRecordGpu& record) {
  validate_postsolve_tape(record.tape);
  if (record.structural_primal_recoveries.size() >
      ...
      record.antipodal_component_recoveries.size() >
          static_cast<std::size_t>(
              std::numeric_limits<std::int32_t>::max())) {
    throw std::runtime_error(
        "primal recovery payload exceeds 32-bit checkpoint range");
  }
  std::vector<std::uint8_t> seen_structural(
      record.structural_primal_recoveries.size(), std::uint8_t{0});
  std::vector<std::uint8_t> seen_antipodal(
      record.antipodal_component_recoveries.size(), std::uint8_t{0});
  std::int32_t previous_tape_position = 0;
  const std::int32_t tape_count =
      static_cast<std::int32_t>(record.tape.types.size());
  for (const PrimalRecoveryCheckpoint& checkpoint :
       record.primal_recovery_timeline) {
    if (checkpoint.tape_position < previous_tape_position ||
        checkpoint.tape_position > tape_count) {
      throw std::runtime_error(
          "primal recovery checkpoints are not chronological");
    }
    previous_tape_position = checkpoint.tape_position;
    if (checkpoint.kind == PrimalRecoveryKind::StructuralL1) {
      if (checkpoint.payload_index < 0 ||
          checkpoint.payload_index >=
              static_cast<std::int32_t>(seen_structural.size()) ||
          seen_structural[static_cast<std::size_t>(
              checkpoint.payload_index)] != std::uint8_t{0}) {
        throw std::runtime_error(
            "invalid or duplicate structural recovery checkpoint");
      }
      seen_structural[static_cast<std::size_t>(checkpoint.payload_index)] =
          std::uint8_t{1};
      validate_structural_recovery(
          record.structural_primal_recoveries[static_cast<std::size_t>(
              checkpoint.payload_index)],
          record.n0);
    } else if (checkpoint.kind ==
               PrimalRecoveryKind::AntipodalComponent) {
      if (checkpoint.payload_index < 0 ||
          checkpoint.payload_index >=
              static_cast<std::int32_t>(seen_antipodal.size()) ||
          seen_antipodal[static_cast<std::size_t>(
              checkpoint.payload_index)] != std::uint8_t{0}) {
        throw std::runtime_error(
            "invalid or duplicate antipodal recovery checkpoint");
      }
      seen_antipodal[static_cast<std::size_t>(checkpoint.payload_index)] =
          std::uint8_t{1};
      validate_antipodal_recovery(
          record.antipodal_component_recoveries[static_cast<std::size_t>(
              checkpoint.payload_index)],
          record.n0);
    } else {
      throw std::runtime_error("unknown primal recovery checkpoint kind");
    }
  }
  for (const std::uint8_t seen : seen_structural) {
    if (seen == std::uint8_t{0}) {
      throw std::runtime_error(
          "structural recovery payload is missing a checkpoint");
    }
  }
  for (const std::uint8_t seen : seen_antipodal) {
    if (seen == std::uint8_t{0}) {
      throw std::runtime_error(
          "antipodal recovery payload is missing a checkpoint");
    }
  }
}
...
void postsolve_restore_structural_primal_gpu(
  ...
  for (auto it = recoveries.rbegin(); it != recoveries.rend(); ++it) {
    restore_structural_primal_step(x_org, *it, original_l);
  }
...
GpuPostsolveResult postsolve_gpu(double* x_red,
                                 ...
                                 const LPInfoGpu* original_model,
                                 const double* original_l) {
  validate_primal_recovery_timeline(record);
  ...
  if (record.tape_gpu.record_count > 0) {
    if (record.tape_gpu.record_count !=
            static_cast<std::int32_t>(record.tape.types.size()) ||
        record.tape_gpu.index_count !=
            static_cast<std::int32_t>(record.tape.indices.size()) ||
        record.tape_gpu.value_count !=
            static_cast<std::int32_t>(record.tape.vals.size())) {
      throw std::runtime_error(
          "device postsolve tape does not mirror the canonical CPU tape");
    }
    borrow_postsolve_tape(record.tape_gpu, device_tape);
  } else {
    upload_postsolve_tape(record.tape, device_tape);
  }
  std::int32_t range_end = device_tape.count;
  for (auto checkpoint_it = record.primal_recovery_timeline.rbegin();
       checkpoint_it != record.primal_recovery_timeline.rend();
       ++checkpoint_it) {
    replay_postsolve_tape_range(
        result.x_org, result.y_org, result.z_org, z_retrieved, device_tape,
        original_model, checkpoint_it->tape_position, range_end, true, false,
        1.0e-7);
    if (checkpoint_it->kind == PrimalRecoveryKind::StructuralL1) {
      restore_structural_primal_step(
          result.x_org,
          record.structural_primal_recoveries[static_cast<std::size_t>(
              checkpoint_it->payload_index)],
          original_l);
    } else {
      restore_antipodal_component(
          result.x_org,
          record.antipodal_component_recoveries[static_cast<std::size_t>(
              checkpoint_it->payload_index)]);
    }
    range_end = checkpoint_it->tape_position;
  }
  replay_postsolve_tape_range(result.x_org, result.y_org, result.z_org,
                              z_retrieved, device_tape, original_model, 0,
                              range_end, true, false, 1.0e-7);
  replay_postsolve_tape(
      result.x_org, result.y_org, result.z_org, z_retrieved, device_tape, original_model, false, true, 1.0e-7);
