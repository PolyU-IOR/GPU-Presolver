...
void test_deletes_duplicate_row_and_tightens_representative() {
  ...
  gpu_presolver::presolve::apply_rule_duplicate_rows(plan, lp, stats, params);
  ...
  assert((plan.tape.types == std::vector<std::int32_t>{
                                 static_cast<std::int32_t>(PostsolveReductionType::DuplicateRow)}));
...
void test_detects_disjoint_duplicate_row_intervals() {
  using gpu_presolver::presolve::LPInfoGpu;
  using gpu_presolver::presolve::PresolveParams;
  using gpu_presolver::presolve::PresolvePlanGpu;
  using gpu_presolver::presolve::PresolveStatsGpu;
...
int main() {
  test_deletes_duplicate_row_and_tightens_representative();
  test_detects_disjoint_duplicate_row_intervals();
  std::cout << "test_rule_duplicate_rows passed\n";
  return 0;
}
